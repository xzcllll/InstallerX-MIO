...
fun EditPage(
        ...
        LaunchedEffect(true) { viewModel.dispatch(EditViewAction.Init) }
        ...
        Scaffold(
                ...
                LazyColumn(
                        ...
                        item { UseAuthorizerLauncherWidget(viewModel = viewModel) }
                }
        }
}
...
fun DataNameWidget(viewModel: EditViewModel) {
        OutlinedTextField(
                ...
                onValueChange = { viewModel.dispatch(EditViewAction.ChangeDataName(it)) },
                singleLine = true,
...
fun DataDescriptionWidget(viewModel: EditViewModel) {
        TextField(
                ...
                label = { Text(text = stringResource(id = R.string.config_description)) },
...
fun DataAuthorizerWidget(viewModel: EditViewModel) {
        ...
        val data =
                mapOf(
                        ConfigEntity.Authorizer.Global to
                                stringResource(R.string.config_authorizer_global),
...
fun DataCustomizeAuthorizerWidget(viewModel: EditViewModel) {
        ...
        TextField(
                ...
                label = { Text(text = stringResource(R.string.config_customize_authorizer)) },
                value = customizeAuthorizer,
                onValueChange = {
                        viewModel.dispatch(EditViewAction.ChangeDataCustomizeAuthorizer(it))
                },
                maxLines = 8,
...
fun DataInstallModeWidget(viewModel: EditViewModel) {
        ...
        val data =
                mapOf(
                        ConfigEntity.InstallMode.Global to
                                stringResource(R.string.config_install_mode_global),
...
fun DataDeclareInstallerWidget(viewModel: EditViewModel) {
        ...
        if (authorizer == ConfigEntity.Authorizer.Global) authorizer = ConfigUtil.globalAuthorizer
...
@Composable
fun UseAuthorizerLauncherWidget(viewModel: EditViewModel) {
        val useAuthorizerLauncher = viewModel.state.data.useAuthorizerLauncher
        SwitchWidget(
                icon = Icons.AutoMirrored.TwoTone.Launch,
                title = stringResource(R.string.config_use_authorizer_launcher),
                description = stringResource(R.string.config_use_authorizer_launcher_dsp),
                checked = useAuthorizerLauncher,
        ) { viewModel.dispatch(EditViewAction.ChangeDataUseAuthorizerLauncher(it)) }
}
