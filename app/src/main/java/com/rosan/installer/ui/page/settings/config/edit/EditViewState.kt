...
data class EditViewState(val data: Data = Data.build(ConfigEntity.default)) {
        data class Data(
                val name: String,
                ...
                val useAuthorizerLauncher: Boolean,
        ) {
                ...
                fun toConfigEntity(): ConfigEntity =
                        ConfigEntity(
                                ...
                                customizeAuthorizer =
                                        if (this.authorizerCustomize) this.customizeAuthorizer
                                        else "",
                                ...
                                useAuthorizerLauncher = this.useAuthorizerLauncher
                        )

                companion object {
                        fun build(config: ConfigEntity): Data =
                                Data(
                                        name = config.name,
                                        ...
                                        useAuthorizerLauncher = config.useAuthorizerLauncher
                                )
                }
        }
}
